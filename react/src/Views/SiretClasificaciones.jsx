    ...
    <div className="container-fluid px-0" style={{ paddingTop: '50px', background: '#f8f9fa', minHeight: '100vh' }}>
            ...
            <tbody>
              ...
              {clasificaciones.map(c => (
                <tr key={c.id} style={{ transition: 'background 0.2s ease', cursor: 'pointer' }} onMouseEnter={(e) => e.currentTarget.style.background = '#f8f9fa'} onMouseLeave={(e) => e.currentTarget.style.background = '#fff'}>
                  <td style={{ borderBottom: '1px solid #e9ecef', padding: windowWidth < 425 ? '12px 8px' : '16px' }}>{windowWidth < 425 ? (c.title || c.name).substring(0, 15) : (c.title || c.name)}</td>
                  <td style={{ borderBottom: '1px solid #e9ecef', textAlign: 'center', padding: windowWidth < 425 ? '12px 8px' : '16px' }}>
                    <div style={{ display: 'flex', gap: windowWidth < 425 ? 4 : 8, justifyContent: 'center', flexDirection: windowWidth < 425 ? 'column' : 'row' }}>
                      <button className="btn btn-sm" onClick={()=>setEditItem(c)} style={{ background: 'linear-gradient(to right, #681b32, #200b07)', color: '#fff', border: 'none', padding: windowWidth < 425 ? '6px 8px' : '8px 16px', borderRadius: '6px', fontWeight: 500, boxShadow: '0 2px 4px rgba(104, 27, 50, 0.2)', transition: 'transform 0.2s ease', fontSize: windowWidth < 425 ? '11px' : '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: windowWidth < 425 ? 2 : 6 }} onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'} title="Editar">
                        <svg xmlns="http://www.w3.org/2000/svg" width={windowWidth < 425 ? 12 : 16} height={windowWidth < 425 ? 12 : 16} fill="currentColor" viewBox="0 0 16 16">
                          <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                        </svg>
                        {windowWidth < 425 ? '' : 'Editar'}
                      </button>
                      <button className="btn btn-sm" onClick={()=>setToDelete(c)} style={{ background: 'linear-gradient(135deg, #dc3545 0%, #c82333 100%)', color: '#fff', border: 'none', padding: windowWidth < 425 ? '6px 8px' : '8px 16px', borderRadius: '6px', fontWeight: 500, boxShadow: '0 2px 4px rgba(220, 53, 69, 0.2)', transition: 'transform 0.2s ease', fontSize: windowWidth < 425 ? '11px' : '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: windowWidth < 425 ? 2 : 6 }} onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'} title="Eliminar">
                        <svg xmlns="http://www.w3.org/2000/svg" width={windowWidth < 425 ? 12 : 16} height={windowWidth < 425 ? 12 : 16} fill="currentColor" viewBox="0 0 16 16">
      ...
      {addModalOpen && (
        <div className={`modal-backdrop${closingModalIndex === 'add' ? ' closing' : ''}`} style={{ position:'fixed',left:0,top:0,right:0,bottom:0,background:'rgba(0,0,0,0.6)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000 }}>
          <div className={`modal-content${closingModalIndex === 'add' ? ' closing' : ''}`} style={{ width: windowWidth < 1024 ? 'calc(100% - 32px)' : 520, maxWidth: 520, background:'#fff', borderRadius:12, overflow:'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.25)' }}>
      ...
      {editItem && (
        <div className={`modal-backdrop${closingModalIndex === 'edit' ? ' closing' : ''}`} style={{ position:'fixed',left:0,top:0,right:0,bottom:0,background:'rgba(0,0,0,0.6)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000 }}>
          <div className={`modal-content${closingModalIndex === 'edit' ? ' closing' : ''}`} style={{ width: windowWidth < 1024 ? 'calc(100% - 32px)' : 520, maxWidth: 520, background:'#fff', borderRadius:12, overflow:'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.25)' }}>
            <div style={{ background: 'linear-gradient(135deg, #681b32 0%, #200b07 100%)', color: '#fff', padding: '20px 24px' }}>
              <h4 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" viewBox="0 0 16 16" style={{ marginRight: 10, verticalAlign: 'middle' }}>
                  <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
      ...
      {toDelete && (
        <div className={`modal-backdrop${closingModalIndex === 'delete' ? ' closing' : ''}`} style={{ position:'fixed',left:0,top:0,right:0,bottom:0,background:'rgba(0,0,0,0.6)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000 }}>
          <div className={`modal-content${closingModalIndex === 'delete' ? ' closing' : ''}`} style={{ width: windowWidth < 1024 ? 'calc(100% - 32px)' : 480, maxWidth: 480, background:'#fff', borderRadius:12, overflow:'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.25)' }}>
