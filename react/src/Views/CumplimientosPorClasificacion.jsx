...
export default function CumplimientosMesAnio() {
  ...
  const [enteQuery, setEnteQuery] = useLocalStorage('cumplimientos_porClasificacion_enteQuery', '');
  ...
  // compute list of classifications from the data
  const classifications = useMemo(() => {
    const classSet = new Set();
    (entesList || []).forEach(ente => {
      if (ente.classification) classSet.add(ente.classification);
    });
  ...
  useEffect(() => {
    ...
    handleSearch();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [year, month, enteQuery, classification]);
  ...
  return (
    <div ref={containerRef} className="container-fluid px-0" style={{ background: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <header className="text-white text-center py-5" style={{ background: 'linear-gradient(135deg, #681b32 0%, #200b07 100%)', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
        ...
        <p className="lead" style={{ marginBottom: 0, opacity: 0.95 }}>Buscar Cumplimientos por Clasificación de Entes</p>
      </header>
