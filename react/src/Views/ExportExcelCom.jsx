			...
			<table style={{ width:'100%', borderCollapse:'collapse', fontSize:12, marginBottom:32 }}>
				<tbody>
					{aoa.map((row,i) => {
						...
						// Para Resumen: filas 0,1,2,3 ocupan todo el ancho
						if(isResumen && (i===0||i===1||i===2||i===3)){
							shouldMerge = true;
							colSpan = 2;
						}
							...
							<tr key={i}>
								{shouldMerge ? (
									...
									<td colSpan={colSpan} style={{
										...
										padding:'6px 8px',
										background: (isResumen && (i===0||i===1||i===2||i===3)) ||
													(isMatrix && (i===0||i===1)) ? 'linear-gradient(135deg,#2c3e50,#34495e)' :
													'transparent',
										color: (isResumen && (i===0||i===1||i===2||i===3)) ||
									...
									row.map((cell,j) => {
										...
										// Header row styling para resumen
										if(isResumen && i===12){ fontWeight=700; bg='#217346'; color='#fff'; }
